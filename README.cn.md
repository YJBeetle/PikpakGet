...
本地会保留分享中的文件夹层级。例如系列目录是 `Series`、分享根目录是 `象人工作室`、里面还有 `未编号`，文件会保存到 `DIR/Series/象人工作室/未编号/`。分享根目录直接只有一个文件，或直接列出多个文件而没有文件夹时，文件直接放在 `DIR/Series/`。分享标题不会被当作文件夹名；空文件夹也会创建。已下载的平铺文件在下次运行时会按新目录整理，不重新下载。
