...
它要解决的问题是：PikPak 免费账号只有 **6 GB** 云盘空间，而一个分享文件夹动辄
100 GB 到 40 TB。"先全部转存再慢慢下载"这条路根本走不通 —— 所以这个工具**一次只处理
一个文件**：

```
把 1 个文件转存进云盘 → 下载到本地 → 核对字节数 → 永久删除云盘副本
        → 等配额真的回落 → 下一个
```
...
- **比配额还大的文件根本拿不到。** 6 GB 的云盘放不下 7.6 GB 的文件，没有任何工具
...
- **限速是账号级的，不是连接级的。** 长时间跑下来，单连接实测 0.13–0.7 MB/s（会随
  时段下滑）。4 条分段合计只有约 0.25 MB/s，而且 4 段里有 2 段**一个字节都没拿到**
...
按这个规划时间：400 GB 在 0.13–0.6 MB/s 下，大约是一到四周的连续运行。
...
下载完成的文件按**字节数**与云盘清单核对。这是个货真价实的局限，说清楚比含糊过去
好：PikPak 的 `hash` 字段**不是**文件的 SHA-1（实测两者不同），所以没有上游校验和可
以比对。完整性因此只能依靠 TLS 传输加密 + 精确的字节数校验，`.part` 必须收满全部字节
才会改名成正式文件。
...
pikpakget/stream.py    单流断点续传 + 多分段并发
pikpakget/pipeline.py  链接解析、状态日志、配额逻辑、status/inventory
pikpakget/cli.py       参数解析、单实例锁、信号处理
tests/test_pure.py     61 项纯逻辑测试；不涉及账号、不联网
```
...
- **校验只有字节数。** PikPak 不提供可信的下载校验和（见上），所以长度不变的位翻转
  发现不了。传输有 TLS 保证，服务端那份坏副本没法防。
