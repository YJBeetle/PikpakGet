...
本地会保留分享中的文件夹层级。例如系列目录是 `Series`、分享根目录是 `象人工作室`、里面还有 `未编号`，文件会保存到 `DIR/Series/象人工作室/未编号/`。根目录文件夹与子文件夹按同一规则处理：如果根目录同时有文件和文件夹，根目录文件直接放在 `DIR/Series/`，文件夹里的文件放进对应子目录。分享标题不会被当作文件夹名；空文件夹也会创建。已下载的平铺文件在下次运行时会按新目录整理，不重新下载。
