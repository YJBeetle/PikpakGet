...
网盘转存使用 `Pack From Shared` 文件夹。下载启动时若该文件夹有剩余项目，程序会列出并询问是否清理；空间不足时也会再次询问。只有输入“删除”才会永久删除，无人值守或拒绝时继续运行并保留原内容。若待转存文件放不进网盘，且文件夹为空或清理后仍放不下，当前文件会记为失败。清理范围只限 `Pack From Shared`。
