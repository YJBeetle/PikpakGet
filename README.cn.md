...
| `--verify` | 关 | 用云端内容 hash 复核所有已下载文件；只报告不删除，发现不符时退出码 1 |
...
SHA-1"，切块大小由上传者当时用的客户端决定（这里见到的多数是 1 MiB，另一个是
512 KiB），所以 `stream.verify_content` 会按候选尺寸逐个试。哪个尺寸都对不上的文件会被
删掉重下 —— 光核字节数看不见这种损坏：两个写入者写同一段，留下的是"长度正好、中间错
位"的文件。
...
- **hash 规则是逆推出来的，没有文档。** 校验依赖对 `hash` 字段的一份逆向理解（分片
  SHA-1 的 SHA-1，分片大小靠候选列表试）。对不上的文件最多重下 3 次然后报失败 —— 方向
  是保守的，但云端 hash 本身就是旧的时会白烧流量。传输有 TLS 保证，服务端那份自洽的坏
  副本没法防。
